class ScoresController < ApplicationController
	...
	def showCalifARealizar
		...
		@calification = Score.where(creador: @user, realizada: false).reorder(fecha: :desc, hora: :desc)
	end

	def showCalificaciones
		@user= User.find(params[:id])
		@calif_piloto = Score.where(calificado: @user, realizada: true, tipo_calificacion: 'p').reorder(fecha: :desc, hora: :desc)
		@calif_copiloto = Score.where(calificado: @user, realizada: true, tipo_calificacion: 'c').reorder(fecha: :desc, hora: :desc)
	end
